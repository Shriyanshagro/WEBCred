    ...
    if action == 'w':
        ...
        for k, v in genre.items():
            ...
            for train, test in kf.split(tempFeatureSet):
                ...
                AcorrScore = np.corrcoef(
                    np.transpose(score), np.transpose(test_data[:, [-1]])
                ).tolist()[0][1]
                # 49%
                print AcorrScore
        ...
        while True:
            ...
            try:
                ...
                with open(r"normalized/Featureset.pickle",
                          "wb") as output_file:
                    cPickle.dump(Featureset, output_file)

                with open(r"normalized/alexaScoreSet.pickle",
                          "wb") as output_score:
                    cPickle.dump(alexaScoreSet, output_score)

                with open(r"normalized/wotScoreSet.pickle",
                          "wb") as output_score:
        ...
        while True:
            ...
            try:
                sample = getjsonData()
                featurevalue, alexaScore, wotScore = getFeatures([sample],
                                                                 FeaturesName)
                # if checksimiliarData(featurevalue[0]):
                # TODO do we need checksimiliarData here?
                Featureset.append(featurevalue[0])
